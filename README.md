...
本项目实现了两种经典的低秩矩阵填充算法, **Soft-Impute（凸方法）** 和**交替最小二乘（ALS，非凸方法）**，用于处理MovieLens 10M数据集中的用户-物品评分预测问题。通过5倍交叉验证的均方根误差（RMSE）评估模型性能，对比两种方法在稀疏评分矩阵补全任务中的效果。

**本项目针对原算法进行了以下改进：**

**Soft-Impute：** 中心化评分矩阵、采用Randomized SVD

**ALS：** 采用SVD初始化、分块计算residual、改进正则化权重ALS-WR
...
├── /history # 算法运行的历史训练记录
├── get_data.py # 稀疏矩阵存储
├── ALS_softImpute.py # 分别实现凸/非凸方法的main文件
...
结果输出包含各模型RMSE、历史训练记录`/history`和运行时间的CSV文件`matrix_completion_results_final_xxxx.csv`

### 3. 时间评估：time_eval.py

该脚本从历史训练记录中提取关键指标，展示不同模型（ALS/SoftImpute）在训练过程中达到目标精度所需的时间，对比模型性能。

确保`/history`目录中存在符合命名规则的历史文件（格式：`{model_name}_fold{fold}_history.npz`），并调整脚本中`target_mse`目标精度，然后运行脚本：
```bash
python time_eval.py 
```
输出分析结果`time_analysis.csv`和可视化图表`time_comparison.png`
...
1. **中心化处理：** 对评分矩阵进行用户和物品偏差中心化

1. **随机截断奇异值分解（Randomized SVD）**：提高大矩阵计算效率；对中心化稀疏矩阵 $\hat{R}$进行截断SVD，保留前k个奇异值： 
...
4. **预测公式**：
  ...
  > 代码中的`predict`方法通过分块点积实现，`U_sub * V_sub`对应上述内积；通过分块计算实现大矩阵乘法，避免GPU OOM
...
2. **迭代优化**：固定一个因子矩阵，求解另一个因子矩阵，交替进行：
...
根据Netflix Prize LeaderBoard提供的Baseline，全球冠军队伍的最好成绩为0.8567，而Netflix指定的Cinematch为0.9525（见参考资料）。**由于我们采用的算法并未结合更多提升精度和降低时间的tricks，基本还是采用的相对传统和经典的迭代方式，因此我们认为，本实验结果落在   $[0.8567,0.9525)$  这一区间范围内时，即可被认定为正确结果。**

**--> 根据以上标准可以判断：我们的所有结果均已进入了Cinematch的基线（0.9525），并不异常低于Winning Goal（0.8567），因此是一次成功的实验。**
### 2. 时间代价
...
#### （1）添加了更加公平的时间对比方式：两种算法达到同一精度所用时间（target MSE = 0.85）；
...
#### 12.12 ---> 继续尝试减小rank参数，并绘图，进行各模型的时间代价的直观对比
| Model                | Avg RMSE | Avg Time (s) | Fold 1 RMSE | Fold 2 RMSE | Fold 3 RMSE | Fold 4 RMSE | Fold 5 RMSE |
| -------------------- | -------- | ------------ | ----------- | ----------- | ----------- | ----------- | ----------- |
| SoftImpute(rank=50)  | 0.943757 | 203.2843     | 0.945404    | 0.944342    | 0.942373    | 0.944654    | 0.94201     |
| SoftImpute(rank=100) | 0.943757 | 273.0642     | 0.945404    | 0.944342    | 0.942373    | 0.944654    | 0.94201     |
| SoftImpute(rank=150) | 0.943757 | 366.6589     | 0.945404    | 0.944342    | 0.942373    | 0.944654    | 0.94201     |
| SoftImpute(rank=200) | 0.943757 | 418.3841     | 0.945404    | 0.944342    | 0.942373    | 0.944654    | 0.94201     |

| Model         | Avg RMSE | Avg Time (s) | Fold 1 RMSE | Fold 2 RMSE | Fold 3 RMSE | Fold 4 RMSE | Fold 5 RMSE |
| ------------- | -------- | ------------ | ----------- | ----------- | ----------- | ----------- | ----------- |
| ALS(rank=50)  | 0.943756 | 64.78363     | 0.945405    | 0.944343    | 0.942368    | 0.944654    | 0.94201     |
| ALS(rank=100) | 0.943757 | 151.6938     | 0.945405    | 0.944343    | 0.942372    | 0.944654    | 0.942011    |
| ALS(rank=150) | 0.943757 | 302.8155     | 0.945405    | 0.944343    | 0.942371    | 0.944654    | 0.94201     |
| ALS(rank=200) | 0.943757 | 456.1829     | 0.945405    | 0.944343    | 0.942372    | 0.944655    | 0.94201     |
